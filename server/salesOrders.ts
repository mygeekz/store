  ...
  try {
    ...
    for (const it of items) {
      if (it.itemType === 'phone') {
        ...
        // هنگام فروش گوشی (فروش نقدی/فاکتور) وضعیت آن را به «فروخته شده» تغییر داده و تاریخ فروش را ثبت می‌کنیم.
        // اگر گوشی قبلاً مرجوع شده باشد، تاریخ مرجوعی را پاک می‌کنیم تا از نمایش تاریخ مرجوعی در آینده جلوگیری شود.
        await runAsync(
          `UPDATE phones SET status='فروخته شده', saleDate=?, returnDate=NULL WHERE id=?`,
