...
const PhoneSalesReportPage: React.FC = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      const res = await apiFetch(`/api/reports/phone-sales?fromDate=${fromDateShamsi}&toDate=${toDateShamsi}`);
      const json = await res.json();
      if (!res.ok || !json.success) throw new Error(json.message || 'خطا در دریافت گزارش فروش موبایل');
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor('transactionDate', {
        header: 'تاریخ فروش',
        cell: info => formatIsoToShamsi(info.getValue(), 'jD jMMMM jYYYY'),
      }),
      columnHelper.accessor('customerFullName', {
        header: 'مشتری',
        cell: info => info.getValue() || 'مهمان',
      }),
      columnHelper.accessor('phoneModel', {
        ...
        cell: info => (
          <div>
            <p>{info.getValue()}</p>
            <p className="text-xs text-muted" dir="ltr">{info.row.original.imei}</p>
          </div>
        ),
      }),
      columnHelper.accessor('purchasePrice', { header: 'قیمت خرید',  cell: info => pricePlain(info.getValue()) }),
      columnHelper.accessor('totalPrice',   { header: 'قیمت فروش',  cell: info => pricePlain(info.getValue()) }),
      columnHelper.accessor('profit',       { header: 'سود',        cell: info => priceColored(info.getValue()) }),
    ],
    []
  );
  ...
  const table = useReactTable({
    ...
    // ==> پیش‌فرض ۱۰ ردیف در هر صفحه
    initialState: { pagination: { pageIndex: 0, pageSize: 10 } },
  });

  const filteredRows = table.getFilteredRowModel().rows;
  const totalProfit = filteredRows.reduce((s, r) => s + (r.original.profit ?? 0), 0);
  const totalRevenue = filteredRows.reduce((s, r) => s + (r.original.totalPrice ?? 0), 0);
  ...
  const topModels = useMemo(() => {
    ...
    for (const r of reportData) {
      const model = (r.phoneModel || 'نامشخص').trim();
  ...
  const topMax = topModels.length ? Math.max(...topModels.map(x => x.revenue)) : 0;
    ...
    <div className="report-page" dir="rtl">
      ...
      <div className="report-surface report-surface-inner">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    <span className={pill}>
                      ...
                      {count.toLocaleString('fa-IR')} ردیف
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className={`${card} ${cardPad} mb-6`}>
          ...
          <div className="mt-4">
            <SavedViewsBar
              storageKey="app:savedViews:reports:phoneSales:v1"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
          <div className={`${card} ${cardPad}`}>
            <div className="flex items-center justify-between">
              <div className="text-xs font-bold text-slate-600 dark:text-slate-300">تعداد فروش</div>
          ...
          <div className={`${card} ${cardPad}`}>
            <div className="flex items-center justify-between">
              <div className="text-xs font-bold text-slate-600 dark:text-slate-300">مجموع فروش</div>
              <Wallet className="h-4 w-4 text-slate-400" />
            </div>
            <div className="mt-2 text-xl font-extrabold text-slate-900 dark:text-white">{money(totalRevenue)}</div>
          </div>
          <div className={`${card} ${cardPad}`}>
            <div className="flex items-center justify-between">
          ...
          <div className={`${card} ${cardPad}`}>
            <div className="flex items-center justify-between">
              <div className="text-xs font-bold text-slate-600 dark:text-slate-300">میانگین سود</div>
              <Percent className="h-4 w-4 text-slate-400" />
            </div>
            <div className="mt-2 text-xl font-extrabold">{priceColored(avgProfit)}</div>
          </div>
        </div>
        ...
        {topModels.length > 0 && (
          <div className={`${card} ${cardPad} mb-6`}>
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-extrabold text-slate-900 dark:text-white">Top مدل‌های پرفروش</div>
              <span className="text-xs text-slate-500 dark:text-slate-300">بر اساس مبلغ فروش</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              {topModels.map((m, idx) => (
                <div key={m.model} className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-white/5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-xs text-slate-500 dark:text-slate-300">#{idx + 1}</div>
                      <div className="mt-0.5 text-sm font-bold text-slate-900 dark:text-white truncate">{m.model}</div>
                    </div>
                    <div className="text-[11px] text-slate-500 dark:text-slate-300">{m.count.toLocaleString('fa-IR')} فروش</div>
                  </div>
                  <div className="mt-2">
                    <ProgressBar value={m.revenue} max={topMax} />
                  </div>
                  <div className="mt-2 text-xs text-slate-700 dark:text-slate-200">{money(m.revenue)}</div>
        ...
        {isLoading ? (
          ...
          <>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-white/10">
                <thead className="bg-slate-50 sticky top-0 z-10 dark:bg-white/5">
                  {table.getHeaderGroups().map(hg => (
                    <tr key={hg.id}>
                      {hg.headers.map(h => (
                        <th key={h.id} className="px-4 py-3 text-right text-xs font-extrabold text-slate-600 dark:text-slate-200">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between p-4 border-t border-slate-200 text-sm dark:border-white/10">
              ...
              <div className="flex items-center gap-2">
                <span>صفحه</span>
                <strong>{table.getState().pagination.pageIndex + 1} از {table.getPageCount()}</strong>
              </div>
              <select
